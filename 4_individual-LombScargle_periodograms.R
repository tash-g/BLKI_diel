...
load("Workshop/rings_anonymised.RData")
...
period_lmm_signif.plot <- ggplot() +
  ...
  geom_line(data = period_lmm_signif.df, aes(y = predicted, x = col_lat),
            linewidth = 1, col = "darkorange") +
  labs(x = expression("Colony latitude °N"), y = "Period (hours)",
       caption = "") +
  scale_x_continuous(breaks = seq(45, 80, by = 5)) + 
...
### ~ FIGURE S7 ~ Ridgeplot - all periods --------------------------------------

png(file = "Figures/FigureS7_ridgePlot_all.png", width = 9, height = 7, units = "in", res = 600)
period_ridges
dev.off()
