...
# If loading from csv:
# gls_hourly <- read.csv("Data_inputs/BLKI_gls-hourly-behaviour.csv")
...
# *Estimation of individual diel rhythms*

## *A note on the periodograms code*
Lomb-Scargle Periodogram analyses handles NAs, but the input data must be a two column dataframe with date and variable of interest. To capture a rhythm at least double the period length is needed. For 24 h rhythm that means a minimum of 48 h of sampling. Also, at minimum, the sampling frequency has to be half the rate or higher (more frequent) than the period of the rhythm (i.e. for a 24 h rhythm, sampling has to be at least every 12 h).
...
gls_hourly %<>% filter(!is.na(imm.total))
...
for(i in 1:length(acc_List)){
  ...
  res_lsp <- lsp(repeats = 1000, TS$imm.total, to = 48, type = "period", ofac = 5, plot = F)
...
save(DF_lsp, file = "Data_outputs/individual_periodograms_imm.RData")
...
load("Data_outputs/individual_periodograms_imm.RData")
load("Data_inputs/BLKI_metadata_anon.RData")
...
period_ridges <- ggplot() + 
  geom_density_ridges(data = DF_lsp,
                      aes(y = sprintf("%.3f", col_lat), x = Period,
                          fill = col_lat), rel_min_height = 0.001, scale = 1.5, alpha = 0.7) +
  geom_point(data = LSP_Results_All, 
                aes(y = sprintf("%.3f", col_lat), x = Period_median), colour = "magenta4", shape = 18, size = 3) +
  geom_errorbarh(data = LSP_Results_All, 
                aes(y = sprintf("%.3f", col_lat), xmin = Period_median - mad_med, xmax = Period_median + mad_med),
                colour = "magenta4",
                height = 0.2,
                size = 1) +
...
#### Figure 3: Immersion trends by colony
...
```{r FIGURE_3, eval = save_plot}

png("Figures/Figure3_immersion_by_colony.png", 
...
# *Latitudinal patterns in rhythmicity*

## Significant periods with latitude

#### Figure S7: Probability of significant period with colony latitude
...
## Calculate individual ACFs 

```{r strength_of_individual_rhythms}
...
#### Figure S6: Individual ACFs

```{r FIGURE_S6, eval = save_plot}
...
#### Figure 4: Absolute deviation & strength by colony
...
period_glmm_signif.plot <- ggplot() +
  ...
  geom_line(data = period_glmm_signif.df, aes(y = predicted, x = col_lat),
            linewidth = 1, col = "magenta4") +
  labs(x = expression("Colony latitude °N"), y = "Absolute deviation from 12-, 24-, and 48-hour periods", tag = "(A)") +
  scale_x_continuous(breaks = seq(45, 80, by = 5)) + 
  BLKI_theme
...
peak_strength.lmm.plot <- ggplot() +
  ...
  geom_line(data = peak_strength.lmm.df, aes(y = predicted, x = col_lat),
            linewidth = 1, col = "magenta4") +
  labs(x = expression("Colony latitude °N"), y = "Peak strength", tag = "(B)") +
  scale_x_continuous(breaks = seq(45, 80, by = 5)) + 
  BLKI_theme +
  theme(legend.position = "none")

# Save plots
png(file = "Figures/Figure4_period_strength_dev.png", width = 19, height = 9, units = "in", res = 600)
ggpubr::ggarrange(period_glmm_signif.plot, peak_strength.lmm.plot, nrow = 1,
...
## Supplementary: Deviations periods ~ latitude 
...
png(file = "Figures/FigureS8_period_length.png", width = 9, height = 7, units = "in", res = 600)
...
# *Behavioural organisation across the diel cycle*
...
#### Figure S10: Activity by sun elevation
...
#### Figure S11: Sun elevation randomisation output
...
## Activity by time of day and latitude
