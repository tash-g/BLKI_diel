...
```{r extra_trips_from_gls}

# Set parameters for trip identification
rolling.width <- 20
thresh.immersion <- 5 #15
...
for (i in 1:length(gls_files)) {
  ...
  for (k in 1:length(myKits)) {
    ...
    total.dur <- mytrips %>% 
      ...
      group_by(date, hour) %>%
      summarise(total_duration = n(), .groups = "drop")
    ...
    ## Calculate proportional attendance
    att.df <- merge(att.df, total.dur, by = c("date", "hour"), all.x = T)
    att.df %<>% mutate(prop_att = mins_att / total_duration) %>% arrange(datetime)
    ...
    hourly_behaviour %<>%
      mutate(activity.mins = round(activity.mins - 60*prop_att),
             activity.mins = ifelse(activity.mins < 0, 0, activity.mins),
             activity.mins = round(activity.mins/ 10) * 10)
    ...
    if(nrow(hourly_behaviour) < 60) next 
