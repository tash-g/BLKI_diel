...
- **BLKI_gls-hourly-behaviour.RData** Dataset summarising hourly daily behaviour for each bird. A .csv alternative is also provided. Data columns are as follows:
  ...
  - _year_: Year of record, numeric YYYY
  - _ring_: Factor encoding unique ID of bird (anonymised)
  - _col_lat_: Colony latitude; decimal degrees, WGS84
  - _col_lon_: Colony longitude; decimal degrees, WGS84
  - _datetime_: Date and time of record rounded to the nearest hour; stored as POSIXct (i.e., YYYY-MM-DD HH:MM:SS)
  - _date_: Date of the record in local time; YYYY-MM-DD
  - _hour_: Numeric hour of record in local time; HH (0 - 23)
  - _imm_total_: Numeric total immersion per hour (0 - 1200)
  - _activity_mins_: Total number of minutes in the activity state, maximum 60 minutes
  - _bout_dur.mins_: Total duration of the bout (record) in minutes, maximum 60 minutes
  - _sun_altitude_: Solar elevation angle (degrees) at the recorded local datetime and location. Positive values indicate the sun is above the horizon; negative values indicate it is below the horizon (i.e., night)
  - _datetime_UTC_: Date and time of the record rounded to the nearest hour and converted to UTC; stored as POSIXct (i.e., YYYY-MM-DD HH:MM:SS)
  - _hour_UTC_: Numeric hour of record converted to UTC; HH (0 - 23)
